...
std::ostream &operator<<(ostream &stream, start_time &time_instance) {
...
// route_struct
void route_struct::print_vector(const vector<double> &input_vector) {
    for (auto number : input_vector) {
        std::cout << number << " ";
    }
}
void route_struct::print_vector(const vector<int> &input_vector) {
    for (auto number : input_vector) {
        std::cout << number << " ";
    }
}
std::ostream& operator<<(std::ostream &stream, route_struct &route_instance) {
    stream  << "      attempt: "; route_instance.print_vector(route_instance.attempt); stream << std::endl
            << "     distance: "; route_instance.print_vector(route_instance.distance); stream << std::endl
            << "  moving_time: "; route_instance.print_vector(route_instance.moving_time); stream << std::endl
            << " elapsed_time: "; route_instance.print_vector(route_instance.elapsed_time); stream << std::endl
            << "movement_type: " << "" << std::endl
            << "average_speed: "; route_instance.print_vector(route_instance.average_speed); stream << std::endl
            << "    max_speed: "; route_instance.print_vector(route_instance.max_speed); stream << std::endl
            << "average_tempo: "; route_instance.print_vector(route_instance.average_tempo); stream << std::endl
            << "         date: " << "" << std::endl
            << "          map: " << "" << std::endl;
    return stream;
}
...
start_time parse_string_to_date(std::string& raw_string) { // vb 2021-09-17T17:36:25Z MOET NOG IS OPNIUEW, DE ss_string RESETTEN WERKT NI
    ...
    stringstream ss_string;
    stringstream ss2; // ffs
    stringstream ss3; // too tired for this
...
// CSV PARSER
std::map<int, route_struct> parse_running_csv(const string& filepath) {
